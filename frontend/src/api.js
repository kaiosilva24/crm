...
export const api = {
    ...
    getGreatPagesLogs: () => request('/leads/greatpages-logs'),
    ...
    backupToSupabase: () => request('/settings/backup-to-supabase', { method: 'POST' }),
    getBackups: () => request('/settings/backups'),
    getBackup: (id) => request(`/settings/backups/${id}`),
    deleteBackup: (id) => request(`/settings/backups/${id}`, { method: 'DELETE' }),
    // Wappi
    getWappiSettings: () => request('/settings/wappi'),
    updateWappiSettings: (data) => request('/settings/wappi', { method: 'POST', body: JSON.stringify(data) }),
    getWappiStatus: () => request('/wappi/status'),
    getWappiGroups: () => request('/wappi/groups'),
    importWappiGroup: (data) => request('/wappi/import', { method: 'POST', body: JSON.stringify(data) }),
    // Whapi.Cloud
